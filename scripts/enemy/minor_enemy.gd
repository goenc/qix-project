...
func _attempt_player_hit(from_point: Vector2, to_point: Vector2, attack_radius: float) -> void:
	...
	if trail_hitbox_enabled and player.has_method("get_active_damage_trail_segments"):
		...
		var trail_hit := _find_trail_hit(from_point, to_point, attack_radius, trail_segments, trail_segment_aabbs, swept_aabb)
...
func _find_trail_hit(
	...
	for index in range(trail_segments.size()):
		...
		if !_rects_overlap(swept_aabb, segment_aabb):
			continue
		var contact := _find_segment_segment_contact(from_point, to_point, segment[0], segment[1], attack_radius)
...
func _find_segment_rect_contact(from_point: Vector2, to_point: Vector2, rect: Rect2, padding: float) -> Dictionary:
	...
	return {"hit": true, "distance": from_point.distance_to(contact_point), "point": contact_point}
...
func _find_segment_segment_contact(a0: Vector2, a1: Vector2, b0: Vector2, b1: Vector2, radius: float) -> Dictionary:
	var d1 := a1 - a0
	var d2 := b1 - b0
	var r := a0 - b0
	var a := d1.dot(d1)
	...
	if closest_a.distance_to(closest_b) > radius:
		return {"hit": false}
	return {"hit": true, "distance": a0.distance_to(closest_a), "point": closest_a}
