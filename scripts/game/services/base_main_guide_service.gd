...
func handle_capture_context(capture_context: Dictionary) -> void:
	...
	_sync_guide_partition_fill_entries_after_capture(
...
func build_draw_data() -> Dictionary:
	return {
		"partition_polygons": _collect_guide_partition_rects(),
		"segments": _collect_guide_draw_segments()
	}
...
func _collect_dirty_guide_indices_from_capture_context(capture_context: Dictionary) -> Array[int]:
	...
	for index in candidate_indices:
		if _guide_segment_overlaps_capture_delta(_main.guide_segments[index], captured_rects, inactive_rects, epsilon):
...
func _append_axis_index_candidates_from_rect(rect: Rect2, epsilon: float, candidate_index_set: Dictionary) -> void:
	...
	_append_axis_index_candidates_in_range(
		_main.vertical_guide_axis_keys,
		_main.vertical_guide_indices_by_x,
		min_x,
		max_x,
		true,
		candidate_index_set
	...
	_append_axis_index_candidates_in_range(
		_main.horizontal_guide_axis_keys,
		_main.horizontal_guide_indices_by_y,
		min_y,
		max_y,
		false,
		candidate_index_set
...
func _append_axis_index_bucket_candidates(
	...
	for raw_index in bucket:
		...
		if index < 0 or index >= _main.guide_segments.size():
			continue
...
func _clear_guide_axis_indices() -> void:
	_main.vertical_guide_indices_by_x.clear()
	_main.horizontal_guide_indices_by_y.clear()
	_main.vertical_guide_axis_keys.clear()
	_main.horizontal_guide_axis_keys.clear()


func _rebuild_guide_axis_indices() -> void:
	_clear_guide_axis_indices()
	for index in range(_main.guide_segments.size()):
		_register_guide_axis_index(index, _main.guide_segments[index])
	_rebuild_guide_axis_key_lists()


func _rebuild_guide_axis_key_lists() -> void:
	_main.vertical_guide_axis_keys = _sorted_axis_keys(_main.vertical_guide_indices_by_x)
	_main.horizontal_guide_axis_keys = _sorted_axis_keys(_main.horizontal_guide_indices_by_y)
...
func _register_guide_axis_index(index: int, guide_segment: Dictionary) -> void:
	...
	if absf(direction.y) > 0.0:
		_append_guide_axis_index(_main.vertical_guide_indices_by_x, axis_key, index)
		return
	if absf(direction.x) > 0.0:
		_append_guide_axis_index(_main.horizontal_guide_indices_by_y, axis_key, index)
...
func _rebuild_spatial_caches() -> void:
	_main.claimed_polygon_aabbs = PlayfieldBoundary.build_polygon_aabbs(_main.claimed_polygons)
	_main.inactive_border_segment_aabbs = PlayfieldBoundary.build_segment_aabbs(_main.inactive_border_segments)
...
func _collect_pending_guide_capture_actions(capture_context: Dictionary, actions: Dictionary) -> void:
	...
	for index in pending_indices:
		var resolved_segment := _resolve_pending_guide_segment_for_capture(_main.guide_segments[index])
...
func _apply_capture_guide_actions(capture_actions: Dictionary) -> void:
	...
	for update in confirm_updates:
		...
		if typeof(update) != TYPE_DICTIONARY:
			continue
		_main.guide_segments[index] = update.get("segment", _main.guide_segments[index])
	...
	for raw_index in reresolve_indices:
		...
		if index < 0 or index >= _main.guide_segments.size():
			continue
		_main.guide_segments[index] = _reresolve_guide_segment_for_capture(_main.guide_segments[index])

	var remove_indices := _sort_unique_descending_indices(capture_actions.get("remove", []))
	for index in remove_indices:
		if index < 0 or index >= _main.guide_segments.size():
			continue
		_main.guide_segments.remove_at(index)
...
func _collect_guide_partition_rects() -> Array[PackedVector2Array]:
	...
	for entry in _main.guide_partition_fill_entries:
		var entry_key := _extract_guide_partition_entry_storage_key(entry)
		if entry_key.is_empty() or !_main.guide_partition_fill_polygons_by_key.has(entry_key):
			continue
		var fill_polygons: Array = _main.guide_partition_fill_polygons_by_key[entry_key]
...
func _sync_guide_partition_fill_entries_after_capture(
	...
	if horizontal_outer_segments.is_empty():
		_main.guide_partition_fill_entries.clear()
		_main.guide_partition_fill_polygons_by_key.clear()
		_main.guide_partition_fill_entry_key_sequence = 0
		return
...
func _upsert_guide_partition_fill_entry(entry: Dictionary, touched_pair_keys: Dictionary) -> void:
	...
	if entry_index >= 0:
		_main.guide_partition_fill_entries[entry_index] = entry
		return
	_main.guide_partition_fill_entries.append(entry)
...
func _find_guide_partition_fill_entry_index(entry_to_match: Dictionary) -> int:
	...
	if !entry_key.is_empty():
		for index in range(_main.guide_partition_fill_entries.size()):
			var entry: Dictionary = _main.guide_partition_fill_entries[index]
...
func _allocate_guide_partition_fill_entry_key(pair_key: String) -> String:
	_main.guide_partition_fill_entry_key_sequence += 1
	return "%s#%d" % [pair_key, _main.guide_partition_fill_entry_key_sequence]
...
func _remove_guide_partition_fill_entries_for_guide_interval(
	...
	if interval_key.is_empty():
		return removed_partition_keys
	for index in range(_main.guide_partition_fill_entries.size() - 1, -1, -1):
		var entry: Dictionary = _main.guide_partition_fill_entries[index]
		if _is_guide_partition_fill_entry_persistent(entry):
			continue
		var left_interval_key := _stringify_value(entry.get("left_interval_key", ""), "")
		...
		if left_interval_key == interval_key or right_interval_key == interval_key:
			...
			_main.guide_partition_fill_entries.remove_at(index)
			continue
...
func _collect_horizontal_outer_loop_segments(epsilon: float) -> Array[Dictionary]:
	...
	for index in range(_main.current_outer_loop.size()):
		var start: Vector2 = _main.current_outer_loop[index]
		var end: Vector2 = _main.current_outer_loop[(index + 1) % _main.current_outer_loop.size()]
...
func _collect_unique_active_vertical_guides(horizontal_outer_segments: Array[Dictionary], epsilon: float) -> Array[Dictionary]:
	...
	for axis_key in _main.vertical_guide_axis_keys:
		if !_main.vertical_guide_indices_by_x.has(axis_key):
			continue
		var bucket: Array = _main.vertical_guide_indices_by_x[axis_key]
		for raw_index in bucket:
			var guide_index := int(raw_index)
			if guide_index < 0 or guide_index >= _main.guide_segments.size():
				continue
			var candidate := _build_vertical_partition_guide_candidate(
				_main.guide_segments[guide_index],
				horizontal_outer_segments,
				epsilon
			)
...
func _erase_guide_partition_fill_results(storage_keys: Array[String]) -> void:
	for storage_key in storage_keys:
		if storage_key.is_empty():
			continue
		_main.guide_partition_fill_polygons_by_key.erase(storage_key)
...
func _refresh_guide_partition_fill_result_for_entry(entry: Dictionary, epsilon: float) -> void:
	...
	var intersected: Array = Geometry2D.intersect_polygons(rect_polygon, _main.remaining_polygon)
	...
	if fill_polygons.is_empty():
		_main.guide_partition_fill_polygons_by_key.erase(storage_key)
		return
	_main.guide_partition_fill_polygons_by_key[storage_key] = fill_polygons
...
func _collect_guide_draw_segments() -> Array[Dictionary]:
	var draw_segments: Array[Dictionary] = []
	var epsilon := _get_guide_epsilon()
	for guide_segment in _main.guide_segments:
		...
		if start.distance_to(end) <= epsilon:
			continue
		var is_vertical := absf(direction.y) > 0.0
		if is_vertical and !_main.show_vertical_guides:
			continue
		if absf(direction.x) > 0.0 and !_main.show_horizontal_guides:
			continue
		...
		draw_segments.append({
			...
			"is_short": _is_guide_segment_within_short_threshold(guide_length, epsilon)
		})
...
func _refresh_guide_segments(apply_capture_correction: bool = false) -> void:
	for index in range(_main.guide_segments.size()):
		if _is_pending_guide_segment(_main.guide_segments[index]):
			continue
		_main.guide_segments[index] = _resolve_guide_segment(_main.guide_segments[index], apply_capture_correction)
...
func _find_first_orthogonal_guide_boundary_segment_on_segment(
	...
	for index in range(_main.claimed_polygons.size()):
		if index < _main.claimed_polygon_aabbs.size() and !PlayfieldBoundary.rects_overlap(search_rect, _main.claimed_polygon_aabbs[index], epsilon):
			continue
...
func _find_first_guide_boundary_hit_on_segment(start: Vector2, ray_end: Vector2, epsilon: float) -> Dictionary:
	...
	for index in range(_main.claimed_polygons.size()):
		if index < _main.claimed_polygon_aabbs.size() and !PlayfieldBoundary.rects_overlap(ray_rect, _main.claimed_polygon_aabbs[index], epsilon):
			continue
		best_hit = _pick_nearest_guide_hit(best_hit, _find_guide_loop_hit(start, ray_end, _main.claimed_polygons[index], epsilon), epsilon)
	return best_hit


func _find_pending_guide_preview_hit(start: Vector2, direction: Vector2, epsilon: float) -> Dictionary:
	var preview_loop: PackedVector2Array = _main.current_outer_loop if _main.current_outer_loop.size() >= 3 else _get_guide_boundary_loop()
	var ray_end := _build_guide_ray_end(start, direction, epsilon)
...
func _build_guide_ray_end(start: Vector2, direction: Vector2, epsilon: float) -> Vector2:
	...
	if absf(direction.x) > 0.0:
		var target_x: float = _main.playfield_rect.end.x + margin if direction.x > 0.0 else _main.playfield_rect.position.x - margin
		return Vector2(target_x, start.y)
	if absf(direction.y) > 0.0:
		var target_y: float = _main.playfield_rect.end.y + margin if direction.y > 0.0 else _main.playfield_rect.position.y - margin
...
func _get_guide_boundary_loop() -> PackedVector2Array:
	if _main.remaining_polygon.size() >= 3:
		return _main.remaining_polygon
	return _main.current_outer_loop
...
func _is_point_in_claimed_region(point: Vector2, epsilon: float) -> bool:
	for index in range(_main.claimed_polygons.size()):
		if index < _main.claimed_polygon_aabbs.size() and !PlayfieldBoundary.point_overlaps_rect(point, _main.claimed_polygon_aabbs[index], epsilon):
			continue
		var polygon: PackedVector2Array = _main.claimed_polygons[index]
		if polygon.size() < 3:
			continue
		if Geometry2D.is_point_in_polygon(point, polygon) or PlayfieldBoundary.is_point_on_loop(polygon, point, epsilon):
			return true
...
func _is_point_on_inactive_border(point: Vector2, epsilon: float) -> bool:
	for index in range(_main.inactive_border_segments.size()):
		if index < _main.inactive_border_segment_aabbs.size() and !PlayfieldBoundary.point_overlaps_rect(point, _main.inactive_border_segment_aabbs[index], epsilon):
			continue
		var segment: PackedVector2Array = _main.inactive_border_segments[index]
...
func _is_point_in_remaining_region(point: Vector2, epsilon: float) -> bool:
	...
	if _main.playfield_rect.size.x <= 0.0 or _main.playfield_rect.size.y <= 0.0:
		return false
	return _main.playfield_rect.has_point(point)
...
func _remove_pending_guides() -> void:
	...
	_main.guide_segments = kept_segments
	_rebuild_guide_axis_indices()
